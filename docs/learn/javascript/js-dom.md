...
🔔注意：ClassName()括号是字符串，且只要写类名不需要写 “.”
...
document.querySelector('css选择器'); // 同时支持css选择器
```

- querySelector 返回指定选择器的第一个元素对象

🔔切记注意：括号里面的**选择器需要加符号 .box #nav**

#### 3.1.5、querySelectorAll（H5新增获取元素方法）

```js
docuemnt.querySelectorAll('.box');
docuemnt.querySelectorAll('css选择器'); // 同时支持css选择器
```
...
利用 console.dir() 打印我们返回的元素对象,更好的查看里面的属性和方法

### 3.2、修改元素内容，样式属性

#### 3.2.1、element.innerText 和 element.innerHTML 都可以修改元素内容
...
#### 3.2.2、修改元素属性 -- 这里的属性可以理解为类似于自定义属性
...
* 添加的都是**行内样式**
* css带”-“的属性要使用**驼峰命名法**，如：div.style.backgroundColor = 'pink';
...
- 如果想要保留原先的类名，我们可以这么做，如：`div.className='first change'; 注意空格`
...
- setAttribute 设置自定义属性与手动给元素行内样式中添加自定义属性，直接在行内样式中显示\_，但通过console.dir()查看时，不存在自定义属性。通过元素对象.xxx方式进行访问时访问不到
